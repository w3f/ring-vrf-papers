...
% Structure of Intro:
% Verifiable random functions are... (definition)
% VRFs are being used for... (e.g., blockchains)
% Highlight that EC-VRF has an RFC (https://datatracker.ietf.org/doc/rfc9381/) and is used in blockchains (Sassafras, Cardano, etc.), and a ring version allows advanced features/functionality, importantly, as a drop-in replacement where EC-VRF is already used.
% VRFs are currently undergoing standardization through the IRTF.  In particular, EC-VRF recently became an RFC.

% But VRFs do not provide anonymity.  For example, in leader election, the identity of the leader is revealed.  It would seem that anonymous VRF solves this, but problem with pseudorandomness (pointed out by Unbiasable VRF paper).
...
% Notes: It seems that anonymous VRFs suffice for leader election, using ephemeral public keys, whi
% Emphasize privacy-enhancing technologies if we are submitting to SAC.
